...
caseList = ['Fuhrman2010_boundaryCondition', 
            'PorousLayer_boundaryCondition']

labelList = ['Fuhrman et al. (2010) boundary condition', 
...
for i_case,case in enumerate(caseList):
    ...
    # main plot
    ax1.plot(u/utauF,z/kn,ls = lineList[Ncase], color = colorList[Ncase], label=labelList[i_case])
    if (Ncase==maxCase-1):
        ax1.plot([0, 100000], [0.00203/kn, 0.00203/kn], ':', color='grey', zorder=0)
        pw   = ax1.scatter(vF,zvF, 150, color='k', marker='+', linewidths=2, zorder=10, label='Fuhrman et al. (2010) experimental data')
    ...
    # insert background
    rect2 = plt.Rectangle(
        (zmX1, zmY1), zmX2_U - zmX1, zmY2 - zmY1, fc="lavender")
    ax2.add_patch(rect2)
...
for i_case,case in enumerate(caseList):
    ...
    # main plot
    ax1.plot(k/utauF**2,z/kn,ls = lineList[Ncase], color = colorList[Ncase], label=labelList[i_case])
    if (Ncase==maxCase-1):
        ax1.plot([0, 100000], [0.00203/kn, 0.00203/kn], ':', color='grey', zorder=0)
        pw   = ax1.scatter(kF,zkF, 150, color='k', marker='+', linewidths=2, zorder=10, label='Fuhrman et al. (2010) experimental data')
        handles, labels = ax1.get_legend_handles_labels()
    ...
    # insert background
    rect2 = plt.Rectangle(
        (zmX1, zmY1), zmX2_tke - zmX1, zmY2 - zmY1, fc="lavender")
    ax2.add_patch(rect2)
...
for i_case,case in enumerate(caseList):
    ...
    # main plot
    ax1.semilogx(omega*nu/utauF**2, z/kn, ls = lineList[Ncase], color = colorList[Ncase], label=labelList[i_case])
    if (Ncase==maxCase-1):
        ax1.plot([0, 100000], [0.00203/kn, 0.00203/kn], ':', color='grey', zorder=0)
    ...
    # insert background
    rect2 = plt.Rectangle(
        (zmX1, zmY1), zmX2_omega - zmX1, zmY2 - zmY1, fc="lavender")
    ax2.add_patch(rect2)
    
    # insert plot
    ax2.semilogx(omega*nu/utau**2, z/kn, ls = lineList[Ncase], color = colorList[Ncase])
    if (Ncase==maxCase-1):
        ax2.plot([0, 100000], [0.00203/kn, 0.00203/kn], ':', color='grey', zorder=1)
    ...
    #save
    fig.savefig(pathFig+fig_name+'_omega.pdf', transparent=True, bbox_inches='tight')
