...
for ind_t,time in enumerate(temps_str):
    ...
    plt.figure(2)
    cont = plt.tricontourf(x0, hlens, alpha, [seuil_alpha-pm_seuil, seuil_alpha+pm_seuil], colors=clmap[ind_t])
    ...
    for j in range(len(yCont)-1):
        intersecBrut = intersect(sonde_plage[0], sonde_plage[1], np.array([xCont[j], yCont[j]]), np.array([xCont[j+1], yCont[j+1]]))
    ...
    if float(time) > 6.5:
        ...
        for i in range(len(val_numBin)-1):
            ...
            if intersec is not None:
                ...
                intersecFit = intersect(sonde_plage[0], sonde_plage[1], np.array([xFit[0], valFit[0]]), np.array([xFit[1], valFit[1]]))
    ...
    plt.scatter(intersecBrut[0], intersecBrut[1], zorder=10, color='green', edgecolor='white', s=80, marker='s')
    
    if ind_t == 1:
        plt.plot(xNumBin, val_numBin, color='orange', marker='o', markersize=3, zorder=8, linewidth=2)
        plt.plot(xFit, valFit, color='red', zorder=9, linewidth=2)
        plt.scatter(intersec[0], intersec[1], zorder=10, color='darkorange', marker='d', edgecolor='white', s=80)
        plt.scatter(intersecFit[0], intersecFit[1], zorder=10, color='red', edgecolor='white', s=80)
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_'+str(ind_t)+'.pdf', bbox_inches='tight')
