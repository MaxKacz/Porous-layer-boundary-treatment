...
nom_legend = ["Fuhrman 3D (interIso V2.0)", "CP 3D (interIso V2.1)", "CP 2D (interIso V0.2.5)", "Données expérimentales"]
colormap = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red']

color_paths = ["blue", "red", "orange", "green"] 
symb_legend = [Line2D([0], [0], label=nom_legend[0], color='b'),
               Line2D([0], [0], label=nom_legend[1], color='r'),
               Line2D([0], [0], label=nom_legend[2], color='orange'),
               Line2D([0], [0], label=nom_legend[3], marker='o', markerfacecolor='none', markeredgecolor='black', linestyle='')]
...
for ind_name,probe_name in enumerate(probe_names):
    ...
    mat = scipy.io.loadmat(path_exp+'U_profiles_'+name_exp+probe_name[-1]+'.mat')
    ...
    for ind_path, path in enumerate(paths):
        ...
        varName, probe_loc, timeProbe, varProbe = ff.readsampling(path=path+postProcessFolder+'\\', 
                             probes_name=probe_name, time_name=timeStep)
        
        probeYLoc = (probe_loc[:,1]-probe_loc[0,1])*np.cos(theta) - (probe_loc[:,0]-probe_loc[0,0])*np.sin(theta)
        ...
        probeYLocPlot = []
        for temps in timePlot:
            ind = np.abs(timeProbe - temps).argmin()   # closest time index to the target time
            Unum = varProbe[ind, varName.index('U')]   # get U
            alphaNum = varProbe[ind, varName.index('alpha.water')]
            
            UPlot = np.array([i for ind_r,i in enumerate(Unum) if alphaNum[ind_r] >= seuil_alpha]) # only data corresponding to a ‘water’ cell is retrieved
            probeYLocPlot.append([i for ind_r,i in enumerate(probeYLoc) if alphaNum[ind_r] >= seuil_alpha])
            
            if np.size(np.shape(UPlot)) > 1 : # test if the vector is empty (not enough water)
                UPlotParr = UPlot[:,0]*np.cos(theta) + UPlot[:,1]*np.sin(theta) # calculate bed parallel component
        ...
        for ind,UNum in enumerate(UnumPlot):
            UExp = UexpPlot[ind]
            UExp_noNan = UExp[~np.isnan(UExp)] # remove nan values for the interpolation
            
            try:
                interpNumU = np.interp(Yexp[0:len(UExp_noNan)], probeYLocPlot[ind], UNum)
            except: # if UExp_noNan is empty
                interpNumU = np.array([])
            
            RMSE_part += sum((UExp_noNan-interpNumU)**2)
            BSS_part += sum((UExp_noNan-np.mean(UExp_noNan))**2)
            WSS_part += sum((abs(UExp_noNan-np.mean(UExp_noNan)) + abs(interpNumU-np.mean(UExp_noNan)))**2)
...
for i in range(np.shape(RMSE)[1]):   
    ...
    ax1.scatter(probe_x, RMSE[:,i], color=colormap[i], marker='+', label=nom_legend[i]+' : mean error = '+str(np.mean(RMSE[:,i])))
    ax1.scatter(0, np.mean(RMSE[:,i]), color=colormap[i], marker='o', edgecolor='black', label='_Hidden label') 
    
    ax2.scatter(probe_x, BSS[:,i], color=colormap[i], marker='+', label=nom_legend[i]+' : mean BSS = '+str(np.mean(BSS[:,i])))
    ax2.scatter(0, np.mean(BSS[:,i]), color=colormap[i], marker='o', edgecolor='black', label='_Hidden label')  
    
    ax3.scatter(probe_x, WSS[:,i], color=colormap[i], marker='+', label=nom_legend[i]+' : mean WSS = '+str(np.mean(WSS[:,i])))
    ax3.scatter(0, np.mean(WSS[:,i]), color=colormap[i], marker='o', edgecolor='black', label='_Hidden label')  
