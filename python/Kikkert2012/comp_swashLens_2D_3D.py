...
# Plot swash lens
plt.figure(1)
plt.scatter(xExpBin, hExpPlotBin, color='red', marker='+', label="Données expérimentales", linewidths=2, s=80)
...
for ind_path, path in enumerate(paths):
    ...
    plt.figure(1)
    plt.plot(xCont, yCont, color=clmap[ind_path], linewidth=taille_plot[ind_path], linestyle=style_trait[ind_path])
...
# Save figure
if sauvegarde:
    plt.savefig(dir_path+'\\'+titre_sauv+'_t='+str(temps)+'.pdf', bbox_inches='tight')
