...
paths_OF = ['Z:\\project\\meige\\2024\\24SWASH\\Maxime\\OpenFOAM\\Simulation\\Kikkert2012\\interIsoFoam\\V2.1\\3D_rug13_CP_lin_h=0.35xd50_h0=0.59_geomCorr\\',
...
for probeNum,probe_name in enumerate(probe_names):
    ...
    # Loading simulation data
        ...
        for ind_t, temps in enumerate(timeProbe):
            ...
            if len(UPlot) > 1:
                UPlotParr = UPlot[:,0]*np.cos(theta) + UPlot[:,1]*np.sin(theta) # calculating bed parallel component
        ...
        plt.plot(timeProbe, UnumPlot, color=clmap[ind_path2], label=nom_plots[ind_path2], linewidth=taille_plot[ind_path2], linestyle=style_trait[ind_path2])
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_sonde'+str(probeNum+1)+'.pdf', bbox_inches='tight')
