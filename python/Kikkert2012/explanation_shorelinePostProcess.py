...
for indFig, tFig in enumerate(temps_fig):
    ...
    for j in range(len(val_exp)-1):
        intersecBrut = intersect(sonde_plage[0], sonde_plage[1], np.array([xExp[j], val_exp[j]]), np.array([xExp[j+1], val_exp[j+1]]))
    ...
    for i in range(len(val_expBin)-1):
        ...
        if intersec is not None:
            ...
            for ind_brut,val_brut in enumerate(val_exp):
                ...
                intersecBrutFit = intersect(sonde_plage[0], sonde_plage[1], np.array([xFit[0], valFit[0]]), np.array([xFit[1], valFit[1]]))
    ...
    if indFig == 1:
        plt.plot(xExpBin, val_expBin, color='orange', marker='o', markersize=3, zorder=8, linewidth=2)
        ...
        plt.scatter(intersecBrutFit[0], intersecBrutFit[1], zorder=10, color='red', edgecolor='white', s=80)
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_'+str(indFig)+'.pdf', bbox_inches='tight')
