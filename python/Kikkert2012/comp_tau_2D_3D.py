...
for probeNum,probe_name in enumerate(probe_names):
    ...
    plt.scatter(t, Tau2, color='darkorange', marker='o', facecolors='none', label="Données expérimentales")
    ...
    for ind_path, path in enumerate(paths):
        ...
        plt.plot(timeList, tauPlot, color=clmap[ind_path], linewidth=taille_plot[ind_path], linestyle=style_trait[ind_path])
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_sonde'+str(probeNum+2)+'.pdf', bbox_inches='tight')
