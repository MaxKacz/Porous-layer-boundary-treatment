...
for ind_path,path in enumerate(paths):
    ...
    if lissage :
        tau_cut_filter = nd.filters.gaussian_filter(tau_cut, [1.5,1.5], mode='nearest', truncate=4)
...
tau_diff = np.abs(np.abs(tau_res[0]) - np.abs(tau_res[1]))/((rho_s-rho_w)*g*D) # calculate Shields number
...
# plot 
if normalise_long == 1:
    im = ax.contourf(temps,x/max(SL) , tau_diff, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1])
    im2 = ax.contour(temps,x/max(SL) , tau_diff, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1], linewidths=0.1)
else:
    im = ax.contourf(temps,x , tau_diff, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1])
    im2 = ax.contour(temps,x , tau_diff, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1], linewidths=0.1)
...
cbar.ax.set_title('$\\left| \\Delta \\theta \\right|$', fontsize=18, usetex=True, position=(0.5,0))
...
if normalise_long == 1:
    ...
    ax.set_yticks(np.array([0, 0.2, 0.4, 0.6, 0.8, 1, 1.1]), ['0', '0.2', '0.4', '0.6', '0.8', '1', ''], fontsize=13)
else:
