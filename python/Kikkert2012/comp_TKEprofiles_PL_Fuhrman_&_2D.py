...
for i_name,probe_name in enumerate(probe_names):
    ...
    # Plot TKE
    plt.figure()
    plt.scatter(TKEexpPlot[0], Yexp, color='red', marker='+', label="Données expérimentales")
    ...
    for ind_path, path in enumerate(paths):
        ...
        for temps in timePlot:
            ...
            kPlot = np.array([i for ind_r,i in enumerate(knum) if alphaNum[ind_r] >= seuil_alpha]) # retrieve the indices of the cells containing water
            probeYLocPlot.append([i for ind_r,i in enumerate(probeYLoc) if alphaNum[ind_r] >= seuil_alpha])
            
            if np.size(np.shape(kPlot)) > 1 : # test if the vector is empty (not enough water)
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_t='+str(timePlot[0])+'_sonde'+str(i_name+1)+'.pdf', bbox_inches='tight')
