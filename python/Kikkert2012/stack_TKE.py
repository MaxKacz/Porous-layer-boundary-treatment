...
for ind_path,path in enumerate(paths):
    ...
    im = ax.contourf(temps,x/max(SL) , k_cut, 200, cmap=clmap, vmin=np.nanmin(k_save), vmax=np.nanmax(k_save))
    ...
    # Save figure
    if sauvegarde:
        plt.savefig(dir_path+'\\'+titre_sauv+'_'+nom_sauv[ind_path]+'.pdf', bbox_inches='tight')
