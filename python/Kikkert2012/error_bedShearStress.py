...
path_exp = 'Z:\\project\\meige\\2024\\24SWASH\\Maxime\\Données Kikkert\\Bed Shear Stress\\'
name_exp = 'IMP_015'
...
for probeNum,probe_name in enumerate(probe_names):
    ...
    mat = scipy.io.loadmat(path_exp+'Tau_'+name_exp+'_PIV'+probe_name[-1]+'.mat')
    ...
    for ind_path, path in enumerate(paths):
        ...
        for temps in timeList:
            ...
            gradUPlot = np.array([i for ind_r,i in enumerate(gradUnum) if alphaNum[ind_r] >= seuil_alpha]) # only data corresponding to a ‘water’ cell is retrieved
            probeYLocPlot.append([i for ind_r,i in enumerate(probeYLoc) if alphaNum[ind_r] >= seuil_alpha])
            alphaPlot = np.array([i for ind_r,i in enumerate(alphaNum) if alphaNum[ind_r] >= seuil_alpha])
            nutPlot = np.array([i for ind_r,i in enumerate(nutNum) if alphaNum[ind_r] >= seuil_alpha])
            ...
            if np.size(gradUPlot) > 1 : # test if the vector is empty (not enough water)
                ...
                tau = 2*(nutPlot + nu_eau*alphaPlot + nu_air*(1-alphaPlot))*(rho_eau*alphaPlot + rho_air*(1-alphaPlot)) * symmGradU
        ...
        RMSE[probeNum, ind_path] = np.sqrt(np.nansum((delta_tau_min)**2)/len(interp_tau))
        BSS[probeNum, ind_path] = 1 - np.nansum((delta_tau_min)**2)/np.nansum((delta_tau_moy2_min)**2)
        WSS[probeNum, ind_path] = 1 - np.nansum((delta_tau_min)**2)/np.nansum((abs(delta_tau_moy2_min) + abs(delta_tau_moy1_min))**2)
...
for i in range(np.shape(RMSE)[1]):   
    ...
    ax1.scatter(probe_x, RMSE[:,i], color=clmap[i], marker='+', label=nom_plots[i]+' : mean error = '+str(np.mean(RMSE[:,i])))
    ax1.scatter(0, np.mean(RMSE[:,i]), color=clmap[i], marker='o', edgecolor='black', label='_Hidden label') 
    
    ax2.scatter(probe_x, BSS[:,i], color=clmap[i], marker='+', label=nom_plots[i]+' : mean BSS = '+str(np.mean(BSS[:,i])))
    ax2.scatter(0, np.mean(BSS[:,i]), color=clmap[i], marker='o', edgecolor='black', label='_Hidden label')  
    
    ax3.scatter(probe_x, WSS[:,i], color=clmap[i], marker='+', label=nom_plots[i]+' : mean WSS = '+str(np.mean(WSS[:,i])))
    ax3.scatter(0, np.mean(WSS[:,i]), color=clmap[i], marker='o', edgecolor='black', label='_Hidden label')  
