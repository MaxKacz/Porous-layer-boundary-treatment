...
nom_plots = ["Référence 2D - density variable (interFoam V2.7.1)", "sed original (sedInterFoam V0.2)", 
...
for i_name,probe_name in enumerate(probe_names):
    ...
    for ind_nom, path in enumerate(paths):
        ...
        plt.plot(timeProbe, surf, color=clmap[ind_nom], label=nom_plots[ind_nom], linewidth=taille_plot[ind_nom], linestyle=style_trait[ind_nom])
        ...
        plt.errorbar(tBarre[ind_nom][i_name], yBarre, xerr=[0], yerr=[0.005], capsize = 4, capthick=3,
                     ecolor=clmap[ind_nom], marker = 'none', linestyle = 'none', zorder=10) #plot error bars
        
    plt.scatter(t, h, color='red', marker='+', label="Données expérimentales", linewidths=2, s=80)
