...
paths_XB = []
paths_OF = ['Z:\\project\\meige\\2024\\24SWASH\\Maxime\\OpenFOAM\\Simulation\\Kikkert2012\\interIsoFoam\\V2.0\\3D_rug13_h0=0.59_geomCorr\\',
...
for probeNum,probe_name in enumerate(probe_names):
    ...
    # Loading simulation data
        varName, probe_loc, timeProbe, varProbe = ff.readsampling(path=curr_path+postProcessFolder+'\\', 
        ...
        for ind_t, temps in enumerate(timeProbe):
            ...
            if len(UPlot) > 1:
                UPlotParr = UPlot[:,0]*np.cos(theta) + UPlot[:,1]*np.sin(theta) # calculate the bed parallel component 
                UnumPlot.append(sum((UPlotParr[1:]*np.transpose(alphaPlot[1:])[0]))/sum(alphaPlot[1:])[0])
            else:
        ...
        RMSE[probeNum, ind_path2] = np.sqrt(np.nansum((interp_U - Uexp)**2)/len(interp_U))
        BSS[probeNum, ind_path2] = 1 - np.nansum((Uexp-interp_U)**2)/np.nansum((Uexp-np.nanmean(Uexp))**2)
        WSS[probeNum, ind_path2] = 1 - np.nansum((Uexp-interp_U)**2)/np.nansum((abs(Uexp - np.nanmean(Uexp)) + abs(interp_U - np.nanmean(Uexp)))**2)
...
for i in range(np.shape(RMSE)[1]):       
    plt.scatter(probe_x, RMSE[:,i], color=clmap[i], marker='+', label=nom_plots[i]+' : mean error = '+str(np.mean(RMSE[:,i])))
    plt.scatter(0, np.mean(RMSE[:,i]), color=clmap[i], marker='o', label='_Hidden label') 
