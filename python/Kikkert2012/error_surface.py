...
for ind_name,probe_name in enumerate(probe_names):
    ...
    for ind_nom, path in enumerate(paths):
        ...
        for i in range(len(timeProbe)):
            ...
            surf[i] = (probe_loc[ind_surf, 1]-probe_loc[0, 1])*np.cos(theta)-(probe_loc[ind_surf, 0]-probe_loc[0, 0])*np.sin(theta)
        ...
        BSS[ind_name, ind_nom] = 1 - sum((h-interpNumH)**2)/sum((h-np.mean(h))**2)
        WSS[ind_name, ind_nom] = 1 - sum((h-interpNumH)**2)/sum((abs(h - np.mean(h)) + abs(interpNumH - np.mean(h)))**2)
...
for i in range(np.shape(RMSE)[1]):   
    ...
    ax1.scatter(probe_x, RMSE[:,i], color=colormap[i], marker='+', label=nom_plots[i]+' : mean error = '+str(np.mean(RMSE[:,i])))
    ax1.scatter(0, np.mean(RMSE[:,i]), color=colormap[i], marker='o', edgecolor='black', label='_Hidden label') 
    
    ax2.scatter(probe_x, BSS[:,i], color=colormap[i], marker='+', label=nom_plots[i]+' : mean BSS = '+str(np.mean(BSS[:,i])))
    ax2.scatter(0, np.mean(BSS[:,i]), color=colormap[i], marker='o', edgecolor='black', label='_Hidden label')  
    
    ax3.scatter(probe_x, WSS[:,i], color=colormap[i], marker='+', label=nom_plots[i]+' : mean WSS = '+str(np.mean(WSS[:,i])))
