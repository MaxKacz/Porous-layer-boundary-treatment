...
nom_plots = ["interIsoFoam Fuhrman (ref 2D) (interIsoFoam V0.2)", 
...
for ind_path, path in enumerate(paths):
    ...
    if not os.path.exists(path+'netcdf\\shoreline.nc'): # if the shoreline was not saved yet
        ...
        for ind_t,time in enumerate(temps_str):
            ...
            if float(time) > 6.5:
                ...
                if inter == 0:
                    list_intersec_num.append(np.array([(xNumBin[i]+xNumBin[i+1])/2,0]))
    ...
    plt.figure(1)
    plt.plot(temps, SL, color=clmap[ind_path], linewidth=taille_plot[ind_path], linestyle=style_trait[ind_path])
