...
for probeNum,probe_name in enumerate(probe_names):
    ...
    for ind_path,curr_path in enumerate(paths):
        ...
        for i in range(len(varProbe)):
            tke = varProbe[i,1]
            alpha = varProbe[i,0]
            ind_eau = np.where(np.diff(np.sign(alpha-seuil_alpha),0) > 0)[0] # retrieve the indices of the cells containing water
        ...
        RMSE[probeNum, ind_path] = np.sqrt(np.nansum((interp_TKE - TKE)**2)/len(interp_TKE))
