...
for ind_path,path in enumerate(paths):
    ...
    for ind_t,t in enumerate(temps):
        ...
        for ind_sonde, sonde in enumerate(list_sondes):
            ...
            print('interpolation : t = '+str(t)+' s, sonde = '+str(ind_sonde+1)+'/'+str(N))
            
            nut_interp = si.griddata((x,y), nut[ind_t], sonde)
            alpha_interp = si.griddata((x,y), alpha[ind_t], sonde)
            gradUSym_interp = si.griddata((x,y), 0.5*(gradU[ind_t,1] + gradU[ind_t,3]), sonde)
            ...
            if len(nut_eau) == 0 or len(gradUSym_eau) == 0: 
                ...
                tau_brut = (2*(nut_eau + nu_eau*alpha_eau + nu_air*(1-alpha_eau))
        ...
        if ind_t % 2 == 0:         
            ...
            if ind_t > 0:
                os.remove(path+'netcdf\\'+titre_sauv+'_bis.nc') # Deletes the existing backup file
            
        else:            
            file = nc.Dataset(path+'netcdf\\'+titre_sauv+'_bis.nc', 'w', format='NETCDF4')
            ...
            os.remove(path+'netcdf\\'+titre_sauv+'.nc') # Deletes the existing backup file
