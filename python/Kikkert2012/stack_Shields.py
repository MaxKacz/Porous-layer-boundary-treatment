...
for ind_path,path in enumerate(paths):
    ...
    # plot 
    if normalise_long == 1:
        im = ax.contourf(temps,x/max(SL) , tau_cut, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1])
        im2 = ax.contour(temps,x/max(SL) , tau_cut, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1], linewidths=0.1)
    else:
        im = ax.contourf(temps,x , tau_cut, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1])
        im2 = ax.contour(temps,x , tau_cut, 1000, cmap=clmap, vmin=clm_val[0], vmax=clm_val[1], linewidths=0.1)
        im3 = ax.contour(temps,x , tau_cut, levels=[-0.1, 0.1], colors='k', linewidths=1)
    ...
    if sauvegarde:
        print('  - sauvegarde...')
        plt.savefig(dir_path+'\\'+titre_sauv+str_tau+'_'+nom_sauv[ind_path]+'.pdf', bbox_inches='tight')
